...
function getGroups() {
	...
	groups.set(
		"devcontainer",
		new Set<string>(["core-tools", "tmux", "delta", "node", "npm-globals", "bat", "gh", "eza"]),
	);
	groups.set("codespaces", new Set<string>(["core-tools", "bat", "eza"]));
	groups.set(
		"popos",
		new Set<string>([
			"core-tools",
...
async function createAndLinkNativefierApp(
	{ appName, displayName, website, frame, internalUrlRegex }: NativefierAppArgs,
) {
	...
	const internalUrls = internalUrlRegex ?? "";
